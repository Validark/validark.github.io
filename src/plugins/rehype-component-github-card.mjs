    ...
    `
      fetch('https://api.github.com/repos/${repo}', { referrerPolicy: "no-referrer" }).then(response => response.json()).then(data => {
        ...
        if (data.license?.spdx_id) {
          document.getElementById('${cardUuid}-license').innerText = ${repo === "Validark/Accelerated-Zig-Parser"} ? "MIT" : ${repo === 'llvm/llvm-project'} ? "Apache License v2.0 with LLVM Exceptions" : data.license?.spdx_id
        } else {
